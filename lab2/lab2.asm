...
Test                 LD        R1,NegX       ; Check for X   
                     ...
                     JSR       mainExit
;
MJ1                  LD        R1,NegC       ; Check for C   
                     ADD       R1,R1,R0
                     BRnp      MJ2
                     JSR       OpClear       ; See Figure 10.27
;
MJ2                  LD        R1,NegPlus    ; Check for +   
                     ADD       R1,R1,R0
                     BRnp      MJ3
                     JSR       OpAdd         ; See Figure 10.10
;
MJ3                  LD        R1,NegMult    ; Check for *   
                     ADD       R1,R1,R0
                     BRnp      MJ4
                     JSR       OpMult        ; See Figure 10.14
;
MJ4                  LD        R1,NegMinus   ; Check for -   
                     ADD       R1,R1,R0
                     BRnp      MJ5
                     JSR       OpNeg         ; See Figure 10.15
;
MJ5                  LD        R1,NegD       ; Check for D   
                     ADD       R1,R1,R0
                     BRnp      MJ6
                     JSR       OpDisplay     ; See Figure 10.26
;
MJ6                  LD        R1,NegMod       ; Check for % 
                     ADD       R1,R1,R0
                     BRnp      MJ7
                     JSR       OpMod
;
MJ7                  LD        R1,NegXOR       ; Check for @
                     ADD       R1,R1,R0
                     BRnp      MJ8
                     JSR       OpXOR
;
...
;
MJ8                  JSR       PushValue     ; See Figure 10.23
;
...
POP            LEA     R0,StackBase
               ...
               RET 
Underflow      ST      R7,popSave     ; TRAP/RET needs R7
               LEA     R0,UnderflowMsg
               PUTS                   ; Print error message.
               LD      R7,popSave     ; Restore R7
               AND     R5,R5,#0  
...
OpDisplay    JSR       POP         ; R0 gets the value to be displayed
             ADD       R5,R5,#0  
             BRnz      DJ1
             JSR       NewCommand  ; POP failed, nothing on the stack.
DJ1          JSR       BinarytoASCII 
             LD        R0,NewlineChar
             OUT
             LEA       R0,ASCIIBUFF
             PUTS
...
Neg10          .FILL  xFFF6
 ...
 OpAdd       JSR     POP             ; Get first source operand.
             ...
             BRp     addExit            ; Branch if not successful.
             ADD     R1,R0,#0        ; Make room for second operand
             JSR     POP             ; Get second source operand.
             ADD     R5,R5,#0        ; Test if POP was successful.
             BRp     addRestore1        ; Not successful, put back first.
             ADD     R0,R0,R1        ; THE Add.
             JSR     RangeCheck      ; Check size of result.
             BRp     addRestore2        ; Out of range, restore both.
             JSR     PUSH            ; Push sum on the stack.
             JSR     NewCommand                     ; On to the next task...
addRestore2     ADD     R6,R6,#-1       ; Decrement stack pointer.
addRestore1     ADD     R6,R6,#-1       ; Decrement stack pointer.
addExit         JSR     NewCommand 
...
RangeCheck    LD        R5,Neg999
              ...
              AND       R5,R5,#0   ; R5 <-- success
              RET
BadRange      ST        R7,checkSave    ; R7 is needed by TRAP/RET
...
OpMult         AND    R3,R3,#0      ; R3 holds sign of multiplier.
               ...
               BRp    multExit          ; Failure
               ADD    R1,R0,#0      ; Make room for next POP
               JSR    POP           ; Get second source operand
               ADD    R5,R5,#0      ; Test for successful POP
               BRp    multRestore1      ; Failure; restore first POP
...
PushMult       JSR    PUSH          ; Push product on the stack.
               JSR     NewCommand 
multRestore2       ADD    R6,R6,#-1     ; Adjust stack pointer.
multRestore1       ADD    R6,R6,#-1     ; Adjust stack pointer.
multExit           JSR     NewCommand    
