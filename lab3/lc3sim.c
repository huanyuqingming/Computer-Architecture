...
void ADD(int instr){
  int dr = (instr & 0x0E00) >> 9;
  int sr1 = (instr & 0x01C0) >> 6;
  int imm_flag = (instr & 0x0020) >> 5;
...
void BR(int instr){
  int n = (instr & 0x0800) >> 11;
  int z = (instr & 0x0400) >> 10;
  int p = (instr & 0x0200) >> 9;
  int offset = (instr & 0x01FF);
  offset = SEXT(offset, 9);
...
void JMP(int instr){
  int base_r = (instr & 0x01C0) >> 6;
  NEXT_LATCHES.PC = CURRENT_LATCHES.REGS[base_r];
}

void JSR(int instr){
  int long_flag = (instr & 0x0800) >> 11;
  int base_r = (instr & 0x01C0) >> 6;
  int offset = (instr & 0x07FF);
...
void LD(int instr){
  ...
  int offset = (instr & 0x01FF);
  offset = SEXT(offset, 9);
  int address = NEXT_LATCHES.PC + offset;
...
void LDI(int instr){
  ...
  int offset = (instr & 0x01FF);
  offset = SEXT(offset, 9);
  int address = MEMORY[NEXT_LATCHES.PC + offset];
  NEXT_LATCHES.REGS[dr] = MEMORY[address];
...
void LDR(int instr){
  int dr = (instr & 0x0E00) >> 9;
  int base_r = (instr & 0x01C0) >> 6;
  int offset = (instr & 0x003F);
  offset = SEXT(offset, 6);
  int address = CURRENT_LATCHES.REGS[base_r] + offset;
  NEXT_LATCHES.REGS[dr] = MEMORY[address];
...
void ST(int instr){
  int sr = (instr & 0x0E00) >> 9;
  int offset = (instr & 0x01FF);
  offset = SEXT(offset, 9);
  int address = NEXT_LATCHES.PC + offset;
  MEMORY[address] = CURRENT_LATCHES.REGS[sr];
  MEMORY[address] = (CURRENT_LATCHES.REGS[sr] & 0xFFFF);
}
